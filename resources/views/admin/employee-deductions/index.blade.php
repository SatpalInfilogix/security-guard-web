                ...
                <div class="col-12">
                    <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                        ...
                        <div class="page-title-right">
                            <a href="#" id="exportBtn" class="btn btn-primary"><i class="bx bx-export"></i>Employee
                                Deduction Export</a>
                            ...
                            @can('create employee deduction')
                                <a href="{{ route('employee-deductions.create') }}" class="btn btn-primary">Add New Employee
                                    Deduction</a>
                            @endcan
                        </div>
                ...
                <div class="col-md-12">
                    <form id="filterForm" method="GET">
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <input type="text" name="search_document_date" class="form-control date_of_separation"
                                    placeholder="Search by Document Date" value="{{ request('search_document_date') }}"
                                    id="search_document_date">
                            </div>

                            <div class="col-md-2">
                                <input type="text" name="search_period_date" class="form-control date_of_separation"
                                    placeholder="Search by Period Date" value="{{ request('search_period_date') }}"
                                    id="search_period_date">
                            </div>
            ...
            let deductionTable = $('#deduction-list').DataTable({
                ...
                columns: [{
                    ...
                    render: function(data, type, row) {
                        let editUrl = "{{ route('employee-deductions.edit', ':id') }}".replace(
                            ':id', row.id);
                        let deleteUrl = "{{ route('employee-deductions.destroy', ':id') }}"
                            .replace(':id', row.id);
                        ...
                        @can('edit employee deduction')
                            buttons +=
                                `<a href="${editUrl}?page=${deductionTable.page() + 1}&search_name=${encodeURIComponent($('#search_name').val())}&search_type=${encodeURIComponent($('#search_type').val())}&search_document_date=${encodeURIComponent($('#search_document_date').val())}&search_period_date=${encodeURIComponent($('#search_period_date').val())}" class="btn btn-sm btn-primary me-1"><i class="bx bx-edit"></i></a>`;
                        @endcan
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            ...
                            success: function(response) {
                                Swal.fire(
                                    'Deleted!', `${source} has been deleted.`,
                                    'success'
                                );
