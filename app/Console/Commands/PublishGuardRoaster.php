...
class PublishGuardRoaster extends Command
{
    ...
    protected function calculateLeaveDetails($userId, $previousStartDate, $previousEndDate)
    {
        ...
        // Get leave type used in current period
        $leaveTypeUsed = Leave::where('guard_id', $userId)
            ->where('status', 'Approved')
            ->whereBetween('date', [$previousStartDate, $previousEndDate])
            ->value('leave_type');

        // Set different limits based on leave type
        if ($leaveTypeUsed === 'Sick Leave') {
            $baseYearlyLimit = (int) setting('sick_leaves') ?: 10;
        } elseif ($leaveTypeUsed === 'Vacation Leave') {
            $baseYearlyLimit = (int) setting('vacation_leaves') ?: 10;
        } elseif ($leaveTypeUsed === 'Maternity Leave') {
            $baseYearlyLimit = (int) setting('maternity_leaves') ?: 56;
        }

        // Calculate leaves taken last year (filter by leave type if specified)
        $usedLeavesLastYear = Leave::where('guard_id', $userId)
            ->where('status', 'Approved')
            ->when($leaveTypeUsed, function ($query) use ($leaveTypeUsed) {
                return $query->where('leave_type', $leaveTypeUsed);
            })
            ->whereYear('date', $previousYear)
            ->count();
        ...
        $lastDayOfDecember = Carbon::createFromDate($year, 12, 31);
        $leavesQuery = Leave::where('guard_id', $userId)
            ->where('status', 'Approved')
            ->when($leaveTypeUsed, function ($query) use ($leaveTypeUsed) {
                return $query->where('leave_type', $leaveTypeUsed);
            });
        ...
        if ($leavesCount > 0) {
            $approvedLeaves = Leave::where('guard_id', $userId)
                ->where('status', 'Approved')
                ->when($leaveTypeUsed, function ($query) use ($leaveTypeUsed) {
                    return $query->where('leave_type', $leaveTypeUsed);
                })
        ...
        $leaveEncashmentAmount = 0;
        // Calculate encashed leaves and amount (unchanged)
        $leaveEncashments = GuardLeaveEncashment::where('guard_id', $userId)
            ->whereBetween('created_at', [$previousStartDate, $previousEndDate])
            ->get();
